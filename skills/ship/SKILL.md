...
Pick the reviewer, most specific first:

1. **A repo-local review skill** (one generated by `build-project-review`, if the repo has one). Prefer it — it encodes what *this* project's maintainers actually care about, which a generic reviewer cannot know.
2. Otherwise **`mattpocock-skills:code-review`**. It reviews the changes since the base along two axes — Standards (does it follow the repo's documented conventions?) and Spec (does it match what the issue asked for?) — in parallel fresh sub-agents. That two-axis, fresh-context shape is exactly what this step needs, so `ship` calls it rather than reimplementing a reviewer.

Feed the reviewer the base ref (so it diffs `base...HEAD`), the issue/spec text, and the repo's `AGENTS.md` / `CONTEXT.md` if present. Surface what it finds to the user before committing.

> The built-in `/code-review` is a *different* tool — a fast, on-demand bug-and-cleanup pass the user runs by hand, with effort levels and `--fix`. It is not part of `ship`. Three things called some flavour of "code review" now coexist; they are a stack (project-local → Standards+Spec → manual bug hunt), not competitors.
